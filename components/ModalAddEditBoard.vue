...
import { httpPut, httpPost, httpErrMsg } from '../services/httpClient';
    ...
    export default {
        ...
        computed: {
            ...
            selectedId() {
                return this.$store.state.modals.editBoard.boardSelected
            }
        },
        created() {
            if (this.mode === 'editBoard') {
                const boardSelected = this.$store.state.userData.boards.find(board => board.id === this.selectedId)
        ...
        methods: {
            ...
            async boardSubmit() {
                ...
                const columns = this.board.columns.value.filter(column => column !== '');
                if (!name || !columns[0]) {
